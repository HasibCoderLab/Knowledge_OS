...
export const Library: React.FC = () => {
  ...
  return (
    <motion.div initial="hidden" animate="visible" variants={containerVariants} className="space-y-8">
      ...
      >
        <div>
          <p className="text-xs font-semibold uppercase tracking-widest text-indigo-600 dark:text-indigo-400 mb-1">Your Collection</p>
      ...
      <motion.div
        variants={{ hidden: { opacity: 0, y: -8 }, visible: { opacity: 1, y: 0, transition: { duration: 0.35, delay: 0.1, ease: 'easeOut' as const } } }}
      >
        <LibraryFilters
      ...
      {filteredBooks.length === 0 ? (
        ...
        <motion.div variants={containerVariants} className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-5 md:gap-6">
      ...
      {(() => {
        ...
        if (!book) return null;
        return (
          <Modal isOpen={true} onClose={() => setSelectedBook(null)} title={book.title} size="lg">
            <div className="space-y-6">
              <div className="flex flex-col sm:flex-row gap-6">
                <div className="w-full sm:w-40 shrink-0">
                  <div className="aspect-[3/4] rounded-xl overflow-hidden bg-slate-100 dark:bg-slate-800 shadow-md">
                    {book.coverUrl ? (
                      ...
                      <div className="flex items-center justify-center h-full"><BookOpen size={32} className="text-slate-300 dark:text-slate-600" /></div>
                    )}
                  </div>
                </div>
                <div className="flex-1 min-w-0 space-y-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <Badge variant={statusBadgeConfig[book.status as keyof typeof statusBadgeConfig]?.variant ?? 'info'}>
                      {statusBadgeConfig[book.status as keyof typeof statusBadgeConfig]?.label ?? book.status}
                    </Badge>
                    {book.rating && (
                      <div className="flex items-center gap-1 px-2 py-0.5 bg-amber-50 dark:bg-amber-900/20 rounded-lg text-[11px] font-semibold text-amber-700 dark:text-amber-400">
                        <Star size={11} className="fill-amber-400 text-amber-400" /> {book.rating}/5
                      </div>
                    )}
                  </div>
                  {book.tags && book.tags.length > 0 && (
                    <div className="flex flex-wrap gap-1.5">
                      {book.tags.map((tag) => (
                        <span key={tag} className="px-2 py-0.5 bg-slate-100 dark:bg-slate-800 rounded-md text-[10px] font-medium text-slate-500 dark:text-slate-400">#{tag}</span>
                      ))}
                    </div>
                  )}
                </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                <div>
                  <p className="text-[10px] font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500 mb-1">Pages</p>
                  <p className="text-sm font-semibold text-slate-900 dark:text-slate-100">{book.totalPages}</p>
                </div>
                {book.startDate && (
                  <div>
                    <p className="text-[10px] font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500 mb-1">Started</p>
                    <p className="text-sm font-semibold text-slate-900 dark:text-slate-100">{book.startDate}</p>
                  </div>
                )}
                {book.finishDate && (
                  <div>
                    <p className="text-[10px] font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500 mb-1">Finished</p>
                    <p className="text-sm font-semibold text-slate-900 dark:text-slate-100">{book.finishDate}</p>
                  </div>
                )}
              </div>
              {book.status === 'reading' && (
                <div className="bg-indigo-50 dark:bg-indigo-900/10 rounded-xl p-5 space-y-3">
                  ...
                  {book.currentPage > 0 && (
                    <p className="text-[10px] text-slate-400 dark:text-slate-500">{book.totalPages - book.currentPage} pages remaining</p>
                  )}
                </div>
              )}
            </div>
